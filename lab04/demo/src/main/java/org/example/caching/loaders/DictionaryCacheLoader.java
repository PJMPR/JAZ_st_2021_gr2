...
public class DictionaryCacheLoader implements CacheLoader{
    ...
    private final List<DictionaryProvider> dictionaryProvidersList;

    public DictionaryCacheLoader() {
        cache = Cache.getInstance();
        dictionaryProvidersList = List.of(new DictionaryFileProvider("dictionaries.csv",","));
    }

    public void load(){
        cache.add("dictionaries",dictionaryProvidersList.stream()
