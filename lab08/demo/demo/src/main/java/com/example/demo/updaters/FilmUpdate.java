...
public class FilmUpdate implements Chain {
    ...
    public FilmUpdate(FilmRepository filmRepository, LanguageRepository languageRepository) {
    ...
    public void query(MovieDto movieDto, OMDbDto omDbDto) {
        List<String> dbList = filmRepository.getAllFilms().stream().map(IFilm::getTitle).collect(Collectors.toList());
        List<String> dbLangList = languageRepository.getAllLanguages().stream().map(ILanguages::getName).collect(Collectors.toList());
